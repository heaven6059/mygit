...
<script>
	...
	var subLoadUrl="<@s.url '/stock/getMxList2MxList.htm'/>";
</script>
...
<body onload="$('#mainTab').tabs({tabPosition:'bottom'});">
	...
	<div id="mainTab" class="easyui-tabs" style="height:660px">
		...
		<div id="mainTab1" title="单据清单查询" style="padding:0px;" width='100%' height='100%' >
		     <iframe src="<@s.url "/stock/queryBillsList_tabMainDouble.htm"/>" frameborder="0"  border="0" marginwidth="0" marginheight="0" scrolling="auto"   style="width:100%" height="99%"></iframe>
		</div>
        ...
        <input type='hidden' id='pid' >        
              <div id="toolbarEdit" style="padding:5px;height:auto;background:#F4F4F4">
					<div style="margin-bottom:5px">
						...
						<a href="javascript:closeWindow('主从从表操作');" class="easyui-linkbutton" iconCls="icon-no" plain="true">关闭</a>
					</div>
			  ...
			  <div style='padding:0px;height:auto;width=100%'>
			        <@p.datagrid id="mainDataGridEdit"    defaultColumn=""  
		                   isHasToolBar="false" divToolbar="#toolbarMx"  title='库存明细信息①'   height="300"   onClickRowEdit="true" singleSelect="true" pageSize="10"
			               columnsJsonList="
			                    [ {title:'主键ID',field : 'stocklistid',width:200,rowspan:2,align:'center'},
		          ...
		          jsonExtend="{onSelect:function(rowIndex, rowData){
                            $('#mainDataGridEditMxMx').datagrid({'url':subLoadUrl+'?id='+rowData.stocklistid,'title':\"库存明细信息-②-当前①主键-\"+rowData.stocklistid,'pageNumber':1 });
                   }}"/>
                   
              <#-- 工具条与查询条件域 -->	
				<div id="toolbarMx" style="padding:1px;height:auto">
						 <a href="javascript:addDataGrid()" class="easyui-linkbutton" iconCls="icon-add" plain="true">新增明细(Alt+A)</a>
						<a href="javascript:removeDataGrid()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除明细(Alt+X)</a>
				</div>
			  <#-- 工具条结束 -->	   
                   
                   
                 <div style='padding:0px;height:auto;width=100%'>
			        <@p.datagrid id="mainDataGridEditMxMx"    defaultColumn=""  
		                   isHasToolBar="false" title='库存明细信息-②'   height="224"   onClickRowEdit="true" singleSelect="true" pageSize="10"
			               columnsJsonList="
			                    [ {field:'stocklist2listid',title:'主键ID',width:250,align:'center'},
			                      {field : 'inamount',title : '入库数',width : 200,editor :{type:'numberbox',options:{required:true,missingMessage:'数量必须输入!',validType:'length[0,8]', invalidMessage:'不能超过8位数字' }}},
			                      {field : 'outamount',title : '出库数',width : 200,editor :{type:'numberbox',options:{required:true,missingMessage:'数量必须输入!',validType:'length[0,8]', invalidMessage:'不能超过8位数字' }}}
			                    ]" 
