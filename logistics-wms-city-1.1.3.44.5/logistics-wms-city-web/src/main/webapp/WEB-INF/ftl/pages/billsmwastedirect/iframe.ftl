...
<script type="text/javascript">
	function upload_(obj){
		...
		if(fileName==""){
			...
			parent.billsmwastedirect.loading("show","正在导入......");
	...
	$(function(){
		...
		var msg = "${msg}";
		if(result=="success"){
			parent.billsmwastedirect.loading();
			parent.billsmwastedirect.importSuccess();
		}else {
			parent.billsmwastedirect.loading();
			if(msg != ''){
				parent.alert(msg,2);
...
<body>
	<div id="div_1">
		<form name="fileForm"  id="fileForm" method="post" action="../bill_sm_waste_dtl_direct/importExcel" class="city-form" enctype="multipart/form-data">
			<input type="hidden" name="wasteNo" value="${wasteNo}"/>
