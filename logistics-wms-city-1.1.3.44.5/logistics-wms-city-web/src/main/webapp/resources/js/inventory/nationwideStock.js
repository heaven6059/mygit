...
//单据明细表头
var preColNames=[{title:"商品编码",field:"itemNo",width:80},
...
//加载单据表头
var gridColumnData = {};
nationwideStock.loadGridColumnHead = function(sysNo){
	//请求路径
...
//初始化动态表头
nationwideStock.initGridColumnData = function(sysNo){
	nationwideStock.loadGridColumnHead(sysNo);
    $("#"+gridId).datagrid({
...
//公共加载DATAGRID
nationwideStock.loadGridDataUtil = function(gridId,url,queryParams){
    var tempObj = $('#'+gridId);
...
//初始化单据数据
nationwideStock.initGridDataDetail = function(gridDataId) {
	//序列form转换json
	var fromObjStr=convertArray($('#searchForm').serializeArray());
	var queryMxURL=BasePath+'/con_content/findNationwideStockList';
	var reqParam = eval("(" +fromObjStr+ ")");
	//加载数据
	nationwideStock.loadGridDataUtil(gridDataId,queryMxURL,reqParam);
};


//搜索库存信息
nationwideStock.searchData = function(){
	...
	//初始化动态表头
	nationwideStock.initGridColumnData(sysNo);
	//初始化数据
	nationwideStock.initGridDataDetail('dataGridStock');
};


//清除查询条件
nationwideStock.searchDataClear = function(){
...
//导出
nationwideStock.exportExcel=function(){
...
//初始化品牌
nationwideStock.initBrand = function(){
